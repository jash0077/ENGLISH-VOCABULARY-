...
export default function Home() {
  ...
  useEffect(() => { let active = true; const timer = window.setTimeout(() => { Promise.all([import("@/data/advancedVocabulary"), import("@/data/advancedVocabularyExpansion")]).then(([base, expansion]) => { if (!active) return; setWords([...CORE_WORDS, ...((base as any).ADVANCED_WORDS as readonly Word[]), ...((expansion as any).ADVANCED_WORDS_EXPANSION as readonly Word[])] as readonly Word[]); }).catch(() => { if (active) toast.error("The full word shelf could not load. Core words remain available."); }).finally(() => { if (active) setWordLibraryLoading(false); }); import("@/data/tenseQuizBank").then(quiz => { if (active) setTenseBank((quiz as any).TENSE_QUIZ_BANK as TenseQuizSet[]); }); }, 420); return () => { active = false; window.clearTimeout(timer); }; }, []);
