...
type Mode = "dashboard" | "shelf" | "everyday" | "academic" | "business" | "flashcards" | "quiz" | "tenses" | "shadowing" | "tenseQuiz" | "review";
...
function Shelf({ words, category, setCategory, query, setQuery, setMode, learned, markLearned, favorites, toggleFavorite, difficulty, setDifficulty, lists, setLists, dedicatedCategory }: { words: readonly Word[]; category: string; setCategory: (v: string) => void; query: string; setQuery: (v: string) => void; setMode: (m: Mode) => void; learned: number; markLearned: (w: string) => void; favorites: string[]; toggleFavorite: (w: string) => void; difficulty: DifficultyFilter; setDifficulty: (value: DifficultyFilter) => void; lists: CustomLists; setLists: Dispatch<SetStateAction<CustomLists>>; dedicatedCategory?: "Everyday" | "Academic" | "Business" }) {
  ...
  const createAiSentence = async () => {
    ...
    try {
      const result = await generateSentence.mutateAsync({ word: selectedWord.w, meaning: selectedWord.m, partOfSpeech: selectedWord.p, category });
      setAiSentence({ word: selectedWord.w, ...result });
    } catch {
...
function TenseQuizView({ bank }: { bank: TenseQuizSet[] }) {
...
function DashboardView({ words, learned, missed, favorites, lists, setLists, daily, setDaily, offline, setMode, difficulty, setDifficulty }: { words: readonly Word[]; learned: string[]; missed: MissedQuestion[]; favorites: string[]; lists: CustomLists; setLists: Dispatch<SetStateAction<CustomLists>>; daily: DailyProgress; setDaily: Dispatch<SetStateAction<DailyProgress>>; offline: boolean; setMode: (mode: Mode) => void; difficulty: DifficultyFilter; setDifficulty: (value: DifficultyFilter) => void }) {
...
export default function Home() {
  ...
  return <div className="app-shell"><ProgressRail mode={mode} setMode={setMode} learned={learnedWords.length} streak={streak} missedCount={missed.length} daily={daily} offline={offline} /><main className="main-canvas"><Header mode={mode} setMode={setMode} />{mode === "dashboard" && <DashboardView words={words} learned={learnedWords} missed={missed} favorites={favorites} lists={lists} setLists={setLists} daily={daily} setDaily={setDaily} offline={offline} setMode={setMode} difficulty={difficulty} setDifficulty={changeDifficulty} />}{(mode === "shelf" || dedicatedCategory) && <Shelf words={filtered} category={shelfCategory} setCategory={setCategory} query={query} setQuery={setQuery} setMode={setMode} learned={learnedWords.length} markLearned={markLearned} favorites={favorites} toggleFavorite={toggleFavorite} difficulty={difficulty} setDifficulty={changeDifficulty} lists={lists} setLists={setLists} dedicatedCategory={dedicatedCategory} />}{mode === "flashcards" && <Flashcards words={filtered.length ? filtered : words} markLearned={markLearned} />}{mode === "quiz" && <Quiz words={filtered.length >= 5 ? filtered : words} onFinish={finishQuiz} markLearned={markLearned} />}{mode === "tenseQuiz" && (tenseBank ? <TenseQuizView bank={tenseBank} /> : <div className="mode-view data-loading"><span className="note-label">LOADING STUDY BANK</span><h2>Preparing the tense desk…</h2><p>The 1,200-question bank is loading only when needed.</p></div>)}{mode === "review" && <ReviewMistakes missed={missed} setMissed={setMissed} />}{mode === "tenses" && <TensesView />}{mode === "shadowing" && <ShadowingView />}<footer className="page-footer"><span>VOCAB STUDIO · A SMALL PRACTICE FOR A BIGGER VOCABULARY</span><span>{words.length} words · 5 shelves · <strong>{learnedWords.length} learned</strong></span></footer></main></div>;
}
