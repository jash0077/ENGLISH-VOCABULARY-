...
type Mode = "shelf" | "flashcards" | "quiz" | "tenses" | "shadowing" | "tenseQuiz" | "review";
...
function ProgressRail({ mode, setMode, learned, streak, missedCount }: { mode: Mode; setMode: (m: Mode) => void; learned: number; streak: number; missedCount: number }) {
  const items = [
...
export default function Home() {
  const [mode, setMode] = useState<Mode>("shelf"); const [category, setCategory] = useState("All words"); const [query, setQuery] = useState(""); const [words, setWords] = useState<readonly Word[]>(INITIAL_WORDS as unknown as readonly Word[]); const [tenseBank, setTenseBank] = useState<TenseQuizSet[] | null>(null); const [learnedWords, setLearnedWords] = useState<string[]>(() => JSON.parse(localStorage.getItem("vocab-learned") || "[]")); const [missed, setMissed] = useState<MissedQuestion[]>(() => JSON.parse(localStorage.getItem("tense-missed-questions") || "[]")); const [streak] = useState(4);
  ...
  const filtered = useMemo(() => words.filter(w => (category === "All words" || w.cat === category) && (`${w.w} ${w.m} ${w.cat}`.toLowerCase().includes(query.toLowerCase()))), [words, category, query]);
