...
function Shelf({ words, allWords, category, setCategory, query, setQuery, setMode, learned, markLearned, favorites, toggleFavorite, difficulty, setDifficulty, lists, setLists, dedicatedCategory, sort, setSort }: { words: readonly Word[]; allWords: readonly Word[]; category: string; setCategory: (v: string) => void; query: string; setQuery: (v: string) => void; setMode: (m: Mode) => void; learned: number; markLearned: (w: string) => void; favorites: string[]; toggleFavorite: (w: string) => void; difficulty: DifficultyFilter; setDifficulty: (value: DifficultyFilter) => void; lists: CustomLists; setLists: Dispatch<SetStateAction<CustomLists>>; dedicatedCategory?: "Everyday" | "Academic" | "Business"; sort: ShelfSort; setSort: (value: ShelfSort) => void }) {
...
export default function Home() {
  ...
  const filtered = useMemo(() => { const matching = words.filter(w => (shelfCategory === "All words" || w.cat === shelfCategory) && (difficulty === "All" || (difficulty === "Easy" ? w.dif === 1 : difficulty === "Medium" ? w.dif === 2 : w.dif >= 3)) && (`${w.w} ${w.m} ${w.cat}`.toLowerCase().includes(query.toLowerCase()))); return [...matching].sort((a, b) => shelfSort === "alphabetical" ? a.w.localeCompare(b.w) : 0); }, [words, shelfCategory, difficulty, query, shelfSort]);
