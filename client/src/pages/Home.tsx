...
const CORE_WORDS = [
  ...
  { w: "3-button navigation bar", p: "n.", ph: "three-BUT-uhn nav-uh-GAY-shuhn bar", m: "a traditional system-control setup at the bottom of a smartphone or tablet screen, usually with buttons for Back, Home, and Recent apps.", ex: "My grandfather prefers the 3-button navigation bar because the controls are easy to find.", syn: ["navigation controls", "system navigation", "button navigation"], cat: "Technology", dif: 1 },
...
type ShelfSort = "recent" | "alphabetical";
...
const CATEGORIES = ["All words", "Everyday", "Academic", "Business", "Literary", "Technology", "Advanced"];
...
function ProgressRail({ mode, setMode, learned, streak, missedCount, daily, offline }: { mode: Mode; setMode: (m: Mode) => void; learned: number; streak: number; missedCount: number; daily: DailyProgress; offline: boolean }) {
  ...
  return <aside className={`rail ${mobileOpen ? "mobile-open" : ""}`}>
    <div className="rail-top"><Logo /><button className="icon-btn mobile-menu" onClick={() => setMobileOpen(open => !open)} aria-label={mobileOpen ? "Close menu" : "Open menu"} aria-expanded={mobileOpen}><Menu size={20} /></button></div>
    <div className="rail-label">Study desk</div>
    <nav className="rail-nav" aria-label="Study modes">
      {items.map(({ id, label, icon: Icon }) => <button key={id} onClick={() => { setMode(id); setMobileOpen(false); }} className={`rail-item ${mode === id ? "active" : ""}`}><Icon size={18} /><span>{label}</span>{id === "quiz" && <b>3</b>}{id === "review" && <b>{missedCount}</b>}</button>)}
    </nav>
    <InstallButton />
...
function WordShelfSkeleton() {
  return <div className="shelf-view shelf-loading" aria-busy="true" aria-live="polite">
    <section className="hero-note skeleton-hero"><div><span className="skeleton-line skeleton-kicker" /><span className="skeleton-line skeleton-title" /><span className="skeleton-line skeleton-title short" /><span className="skeleton-line skeleton-copy" /><span className="skeleton-line skeleton-copy short" /></div><div className="skeleton-art" /></section>
    <div className="section-head skeleton-section-head"><div><span className="skeleton-line skeleton-kicker" /><span className="skeleton-line skeleton-heading" /></div><span className="skeleton-search" /></div>
    <div className="skeleton-filter-row"><span /><span /><span /><span /></div>
    <div className="skeleton-word-list">{[0, 1, 2, 3].map(index => <article className="skeleton-word-row" key={index}><span className="skeleton-index" /><div className="skeleton-word-copy"><span className="skeleton-line skeleton-word-title" /><span className="skeleton-line skeleton-word-meaning" /><span className="skeleton-line skeleton-word-meaning short" /><span className="skeleton-line skeleton-word-meta" /></div><div className="skeleton-word-actions"><span /><span /></div></article>)}</div>
    <p className="skeleton-status">Preparing your word shelf…</p>
  </div>;
}

function Shelf({ words, allWords, category, setCategory, query, setQuery, setMode, learned, markLearned, favorites, toggleFavorite, difficulty, setDifficulty, lists, setLists, dedicatedCategory, sort, setSort }: { words: readonly Word[]; allWords: readonly Word[]; category: string; setCategory: (v: string) => void; query: string; setQuery: (v: string) => void; setMode: (m: Mode) => void; learned: readonly string[]; markLearned: (w: string) => void; favorites: string[]; toggleFavorite: (w: string) => void; difficulty: DifficultyFilter; setDifficulty: (value: DifficultyFilter) => void; lists: CustomLists; setLists: Dispatch<SetStateAction<CustomLists>>; dedicatedCategory?: "Everyday" | "Academic" | "Business" | "Literary"; sort: ShelfSort; setSort: (value: ShelfSort) => void }) {
  ...
  const jumpSuggestions = useMemo(() => { const needle = jumpQuery.trim().toLowerCase(); if (!needle) return []; return words.filter(word => `${word.w} ${word.m}`.toLowerCase().includes(needle)).slice(0, 8); }, [words, jumpQuery]);
  const jumpToWord = (word: Word) => { const index = words.findIndex(item => item.w === word.w); if (index < 0) return; virtualizer.scrollToIndex(index, { align: "center", behavior: "smooth" }); setJumpQuery(""); setShowBackToTop(true); setSelectedWord(word); setAiSentence(null); };
  ...
  return <div className="shelf-view">
    ...
    <div className="section-head"><div><span className="note-label">{dedicatedCategory ? `${dedicatedCategory.toUpperCase()} VOCABULARY` : "THE SHELF"}</span><h2>{dedicatedCategory ? `${dedicatedCategory} words` : "Choose a corner"}</h2></div><div className="shelf-search-tools"><div className="search-wrap"><Search size={17} /><input value={query} onChange={e => setQuery(e.target.value)} placeholder="Filter words…" aria-label="Filter words" /></div><div className="jump-search" role="combobox" aria-expanded={jumpSuggestions.length > 0} aria-haspopup="listbox"><Search size={16} /><input value={jumpQuery} onChange={event => setJumpQuery(event.target.value)} onKeyDown={event => { if (event.key === "Enter" && jumpSuggestions[0]) { event.preventDefault(); jumpToWord(jumpSuggestions[0]); } if (event.key === "Escape") setJumpQuery(""); }} placeholder="Jump to a word…" aria-label="Jump to a word" aria-controls="jump-word-results" autoComplete="off" />{jumpSuggestions.length > 0 && <div id="jump-word-results" className="jump-suggestions" role="listbox">{jumpSuggestions.map(word => <button key={word.w} type="button" role="option" onClick={() => jumpToWord(word)}><strong>{word.w}</strong><span>{word.m}</span></button>)}</div>}</div></div></div>
    {dedicatedCategory ? <div className="dedicated-section-note"><BookOpen size={15} /><span>Focused section · {words.length} words in this collection</span><button onClick={() => setMode("shelf")}>View all categories</button></div> : <div className="category-filter-row"><span className="filter-heading">WORD FILTER</span>{["All words", "Everyday", "Academic", "Business", "Literary", "Technology", "Advanced"].map(c => <button key={c} onClick={() => setCategory(c)} className={category === c ? "selected" : ""}>{c}<span>{c === "All words" ? allWords.length : allWords.filter(w => w.cat === c).length}</span></button>)}<span className="filter-divider" /><span className="filter-heading">LEVEL</span>{(["All", "Easy", "Medium", "Hard"] as DifficultyFilter[]).map(item => <button key={item} onClick={() => setDifficulty(item)} className={difficulty === item ? "selected filter-difficulty" : "filter-difficulty"}>{item}</button>)}</div>}
    <div className="shelf-sort-bar"><label htmlFor="word-shelf-sort"><span>ORDER BY</span><select id="word-shelf-sort" value={sort} onChange={event => setSort(event.target.value as ShelfSort)} aria-label="Order word shelf"><option value="recent">Recently added</option><option value="alphabetical">Alphabetical</option></select></label><span>{sort === "alphabetical" ? "A–Z word order" : "Newest words first"}</span></div>
    {words.length ? <><div key={`${category}-${difficulty}-${query}`} ref={listRef} className="word-list word-list-virtual" onScroll={event => setShowBackToTop(event.currentTarget.scrollTop > 520)}><div style={{ height: `${virtualizer.getTotalSize()}px`, position: "relative" }}>{virtualItems.map(item => { const word = words[item.index]; return <article ref={virtualizer.measureElement} data-index={item.index} className="word-row" key={word.w} style={{ position: "absolute", top: 0, left: 0, width: "100%", transform: `translateY(${item.start}px)`, "--delay": "0ms" } as React.CSSProperties}><div className="word-index">{String(item.index + 1).padStart(2, "0")}</div><div className="word-main"><button className="word-title word-title-trigger" onClick={() => { setSelectedWord(word); setAiSentence(null); }} aria-label={`Open meaning for ${word.w}`}><h3>{word.w}</h3><span>{word.p}</span></button><p>{word.m}</p><div className="word-audio"><SpeakButton text={`${word.w}. ${word.m}`} label="Meaning" />{word.ex && <><span className="word-example">Example: {word.ex}</span><SpeakButton text={word.ex} label="Example" /></>}</div><div className="word-meta"><span>/{word.ph}/</span><span className="dot" /> <span>{word.cat}</span></div></div><div className="word-side"><Difficulty level={word.dif} /><button className={`favorite-btn ${favorites.includes(word.w) ? "active" : ""}`} onClick={() => toggleFavorite(word.w)} aria-label={`${favorites.includes(word.w) ? "Remove" : "Add"} ${word.w} ${favorites.includes(word.w) ? "from" : "to"} favorites`}><Heart size={15} fill={favorites.includes(word.w) ? "currentColor" : "none"} /></button>{Object.keys(lists).length > 0 && <select className="word-list-select" defaultValue="" aria-label={`Add ${word.w} to a custom list`} onChange={event => { const name = event.target.value; if (!name) return; setLists(previous => { const next = { ...previous, [name]: previous[name].includes(word.w) ? previous[name] : [...previous[name], word.w] }; localStorage.setItem("vocab-custom-lists", JSON.stringify(next)); return next; }); event.currentTarget.value = ""; toast.success(`Added ${word.w} to ${name}.`); }}><option value="">List</option>{Object.keys(lists).map(name => <option key={name} value={name}>{name}</option>)}</select>}<button className={`learn-btn ${learned.includes(word.w) ? "done" : ""}`} onClick={() => markLearned(word.w)} aria-pressed={learned.includes(word.w)}>{learned.includes(word.w) ? <><Check size={16} /> Learned</> : "Mark learned"}</button></div></article>; })}</div></div><p className="virtual-list-status">Showing {virtualItems.length ? virtualItems[0].index + 1 : 0}–{virtualItems.length ? virtualItems[virtualItems.length - 1].index + 1 : 0} of {words.length} words · scroll to continue</p>{showBackToTop && <button className="word-back-to-top" onClick={() => { listRef.current?.scrollTo({ top: 0, behavior: "smooth" }); virtualizer.scrollToIndex(0, { align: "start", behavior: "smooth" }); setShowBackToTop(false); }} aria-label="Back to top of word shelf"><ArrowUp size={17} /><span>Back to top</span></button>}</> : <div className="word-empty-state"><Search size={22} /><h3>No words match this filter.</h3><p>Try another category, difficulty, or search phrase.</p><button className="soft-btn" onClick={() => { setCategory("All words"); setDifficulty("All"); setQuery(""); }}>Clear filters</button></div>}
...
function Flashcards({ words, markLearned }: { words: readonly Word[]; markLearned: (w: string) => void }) {
  ...
  return <div className="mode-view flashcards-view"><div className="mode-title"><div><span className="note-label">FLASHCARDS · SET A</span><h2>Recall first. Reveal second.</h2><p>Say the meaning out loud, then turn the card to check yourself.</p></div><div className="counter" aria-label={`Card ${index + 1} of ${words.length}`}>{String(index + 1).padStart(2, "0")} <span>/ {String(words.length).padStart(2, "0")}</span></div></div><div className="flashcard-instruction" aria-live="polite"><RotateCw size={15} /> {flipped ? "Tap the card to return to the word" : "Tap the card to reveal the meaning"}</div><div className={`flashcard-wrap ${flipped ? "is-flipped" : ""}`}><button className="flashcard" onClick={toggleCard} aria-label={flipped ? "Show word" : "Reveal definition"} aria-pressed={flipped}><div className="card-face card-front"><div className="card-corner">WORD {String(index + 1).padStart(2, "0")}</div><span className="card-category">{word.cat}</span><h3>{word.w}</h3><span className="phonetic">/{word.ph}/</span><span className="flip-hint"><RotateCw size={14} /> tap to reveal</span></div><div className="card-face card-back"><div className="card-corner">MEANING {String(index + 1).padStart(2, "0")}</div><span className="card-category">{word.p} · {word.cat}</span><h3>{word.m}</h3><div className="example"><span>IN A SENTENCE</span><p>“{word.ex}”</p></div><div className="synonyms"><span>NEARBY WORDS</span><div>{word.syn.map(s => <b key={s}>{s}</b>)}</div></div></div></button></div><div className="flash-actions"><button className="outline-btn" onClick={() => { markLearned(word.w); next(); }}><Check size={16} /> I knew it</button><button className="soft-btn" onClick={next}><ChevronRight size={17} /> Next card</button></div><div className="tip-box"><Lightbulb size={18} /><p><strong>Memory cue</strong> Connect a new word to a sentence you actually might say today.</p></div></div>;
}
...
export default function Home() {
  ...
  useEffect(() => { const needsWordLibrary = mode === "shelf" || mode === "everyday" || mode === "academic" || mode === "business" || mode === "flashcards" || mode === "quiz"; if (!needsWordLibrary || wordLibraryRequested.current) return; wordLibraryRequested.current = true; let active = true; setWordLibraryLoading(true); const timer = window.setTimeout(() => { import("@/data/advancedVocabulary").then(base => { if (!active) return; const baseWords = [...CORE_WORDS, ...((base as any).ADVANCED_WORDS as readonly Word[]) ] as readonly Word[]; setWords(baseWords); setWordLibraryLoading(false); const scheduleIdle = window.requestIdleCallback ?? ((callback: IdleRequestCallback) => window.setTimeout(() => callback({ didTimeout: false, timeRemaining: () => 0 } as IdleDeadline), 240)); scheduleIdle(() => { import("@/data/advancedVocabularyExpansion").then(expansion => { if (active) setWords([...baseWords, ...((expansion as any).ADVANCED_WORDS_EXPANSION as readonly Word[])] as readonly Word[]); }).catch(() => { if (active) toast.error("The extended word shelf could not load. Core words remain available."); }); }); }).catch(() => { if (active) { setWordLibraryLoading(false); toast.error("The word shelf could not load. Core words remain available."); } }); }, 80); return () => { active = false; window.clearTimeout(timer); }; }, [mode]);
  useEffect(() => { if (mode !== "tenseQuiz" || tenseBank) return; let active = true; import("@/data/tenseQuizBank").then(quiz => { if (active) setTenseBank((quiz as any).TENSE_QUIZ_BANK as TenseQuizSet[]); }); return () => { active = false; }; }, [mode, tenseBank]);
  ...
  const filtered = useMemo(() => { const needle = query.trim().toLowerCase(); const matching = words.filter(w => (shelfCategory === "All words" || w.cat === shelfCategory) && (difficulty === "All" || (difficulty === "Easy" ? w.dif === 1 : difficulty === "Medium" ? w.dif === 2 : w.dif >= 3)) && (!needle || `${w.w} ${w.m} ${w.ex} ${w.cat} ${w.syn.join(" ")}`.toLowerCase().includes(needle))); return [...matching].sort((a, b) => shelfSort === "alphabetical" ? a.w.localeCompare(b.w) : 0); }, [words, shelfCategory, difficulty, query, shelfSort]);
  ...
  const changeShelfSort = (value: ShelfSort) => { setShelfSort(value); localStorage.setItem("vocab-shelf-sort", value); };
