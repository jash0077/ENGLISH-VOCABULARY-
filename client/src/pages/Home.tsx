...
function Shelf({ words, category, setCategory, query, setQuery, setMode, learned, markLearned, favorites, toggleFavorite, difficulty, setDifficulty, lists, setLists, dedicatedCategory }: { words: readonly Word[]; category: string; setCategory: (v: string) => void; query: string; setQuery: (v: string) => void; setMode: (m: Mode) => void; learned: number; markLearned: (w: string) => void; favorites: string[]; toggleFavorite: (w: string) => void; difficulty: DifficultyFilter; setDifficulty: (value: DifficultyFilter) => void; lists: CustomLists; setLists: Dispatch<SetStateAction<CustomLists>>; dedicatedCategory?: "Everyday" | "Academic" | "Business" }) {
  ...
  const [aiSentence, setAiSentence] = useState<{ word: string; sentence: string; tip: string; difficulty: "Beginner" | "Intermediate" | "Advanced" } | null>(null);
  ...
  const createAiSentence = async () => {
    ...
    try {
      const result = await generateSentence.mutateAsync({ word: selectedWord.w, meaning: selectedWord.m, partOfSpeech: selectedWord.p, category, difficulty: sentenceDifficulty });
      setAiSentence({ word: selectedWord.w, difficulty: sentenceDifficulty, ...result });
    } catch {
  ...
  return <div className="shelf-view">
    ...
    {selectedWord && <aside className="word-detail-panel" aria-live="polite"><button className="word-detail-close" onClick={() => setSelectedWord(null)} aria-label="Close word meaning">×</button><span className="note-label">WORD NOTE · {selectedWord.cat}</span><div className="word-detail-heading"><div><h2>{selectedWord.w}</h2><span>{selectedWord.p} · /{selectedWord.ph}/</span></div><SpeakButton text={`${selectedWord.w}. ${selectedWord.m}`} label="Listen" /></div><p className="word-detail-meaning">{selectedWord.m}</p><div className="ai-sentence-panel"><div><span className="note-label">AI PRACTICE SENTENCE · {sentenceDifficulty.toUpperCase()}</span><p>{aiSentence?.word === selectedWord.w && aiSentence.difficulty === sentenceDifficulty ? `“${aiSentence.sentence}”` : "Generate a fresh sentence that uses this word naturally."}</p>{aiSentence?.word === selectedWord.w && aiSentence.difficulty === sentenceDifficulty && <small>{aiSentence.tip}</small>}</div><div className="ai-sentence-actions"><label className="sentence-difficulty-control"><span>LEVEL</span><select value={sentenceDifficulty} onChange={event => { const value = event.target.value as "Beginner" | "Intermediate" | "Advanced"; setSentenceDifficulty(value); localStorage.setItem("ai-sentence-difficulty", value); setAiSentence(null); }} aria-label="AI sentence difficulty"><option>Beginner</option><option>Intermediate</option><option>Advanced</option></select></label><button className="soft-btn" onClick={createAiSentence} disabled={generateSentence.isPending}><Sparkles size={15} />{generateSentence.isPending ? "Writing…" : aiSentence?.word === selectedWord.w && aiSentence.difficulty === sentenceDifficulty ? "Generate another" : "Generate with AI"}</button>{aiSentence?.word === selectedWord.w && aiSentence.difficulty === sentenceDifficulty && <SpeakButton text={aiSentence.sentence} label="AI sentence" />}</div></div>{selectedWord.ex && <div className="word-detail-example"><span>SAVED EXAMPLE SENTENCE</span><p>“{selectedWord.ex}”</p><SpeakButton text={selectedWord.ex} label="Example" /></div>}<div className="related-words"><span>RELATED WORDS</span><div>{(selectedWord.syn.length ? selectedWord.syn : ["No related words listed yet"]).map(related => <button key={related} onClick={() => { const next = words.find(item => item.w === related); if (next) setSelectedWord(next); }}>{related}</button>)}</div></div></aside>}
  </div>;
}
